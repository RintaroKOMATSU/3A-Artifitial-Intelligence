...
\section{序論}
本課題では，メタヒューリスティックスを用いた多目的最適化シミュレーターを作成した．MOABC（Multi-Objective Artificial Bee Colony）とMOGWO（Multi-Objective Gray Wolf Optimizer）の2つのアルゴリズムを実装し，2つの関数に対してどちらかのアルゴリズムを選択して最適化を行うことができるようにした．本レポートでは，2つのアルゴリズムの概要，シミュレーターの使用方法，及び性能評価について述べる．
...
\begin{itemize}
    \item \textbf{被覆率（Cover rate: CR）}\\\quad
        被覆率は解の多様性を表す指標である．求めるには，目的関数の最大値と最小値を求め，その間を任意の個数に分割する．そうしてできた小領域をパレート解がカバーしている割合を求めればいい・
    \item \textbf{優越個体割合（Ratio of Non-dominated Individuals: RNI）}\\\quad
        優越個体割合は，異なる手法から得られた2つのパレート最適解のうち，一方の解が他方の解に対して優越している割合を示す指標である．具体的には，手法Aから得られたパレート最適解のうち，手法Bから得られたパレート最適解に対して優越している解の割合を計算する．この割合が高いほど，手法Aの方が手法Bよりも優れていることを示す．反対にこの割合が低ければ手法Bの方が優れていることになる．
\end{itemize}
\subsection{ベンチマーク問題}
評価のためのベンチマークとして[3]にある合わせて6つの最適化問題を使った．以下にそれぞれの問題とパレート最適解を示す．
\vspace{10pt}
...
\begin{flalign*}
    ...
    \mbox{Sub}& \mbox{ject to}  &\\
                        ...
                        & x_i \in [-5,5]\quad (i = 1, \cdots, N = 10)&
\end{flalign*}
最適解は不明
\end{itembox}
